...
def train_rnnsm(data_loader, model, path, cfg, device):
    ...
    for epoch in range(cfg.training.n_epochs):
        ...
        for times, events, lengths, _ in data_loader:
            time_deltas = torch.cat([torch.zeros(times.shape[0], 1, times.shape[2]),
                                     times[:, 1:] - times[:, :-1]],
                                     dim=1)
            o_t = model(events.to(device), time_deltas.to(device), lengths)
...
train_rnnsm(data_loader, model, args.model_path, cfg, device)
